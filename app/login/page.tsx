...
export default async function Login({ searchParams }: { searchParams: Promise<{ error?: string }> }) {
  ...
  return <main className="auth-page"><Link href="/" className="auth-brand">✚ UTI DO NOTEBOOK</Link><form action={signIn} className="auth-card"><p className="eyebrow teal">Área da equipa</p><h1>Entrar no CRM</h1><p>Use o e-mail e a senha cadastrados pela administração.</p><label>E-mail<input name="email" type="email" required /></label><label>Senha<input name="password" type="password" required /></label>{query.error === "unauthorized" ? <small className="login-error">Este acesso é exclusivo da equipa autorizada.</small> : query.error ? <small className="login-error">Não foi possível entrar. Confira os dados.</small> : null}<button className="btn dark">Entrar</button></form></main>;
}
